...
export class DatabaseOperations {
	...
	async addConversation(conversation: GeminiConversationEntry): Promise<number> {
		return this.queue.enqueue(async () => await this.conversations.add(conversation), 'Add Conversation');
	}

	async getConversations(notePath: string): Promise<GeminiConversationEntry[]> {
		return this.queue.enqueue(
			async () => await this.conversations.where('notePath').equals(notePath).toArray(),
			'Get Conversations'
		);
	}
	...
	async clearConversations(notePath: string): Promise<number> {
		return this.queue.enqueue(
			async () => await this.conversations.where('notePath').equals(notePath).delete(),
			'Clear Conversations'
		);
	}
