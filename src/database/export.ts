		...
		return this.queue.enqueue(async () => {
			...
			try {
				...
				if (conversations.length < this.lastConversationCount) {
					console.error(
						`Preventing export: conversation count decreased from ${this.lastConversationCount} to ${conversations.length}`
					);
					return;
				}
			...
			Object.entries(
				conversations.reduce(
					(acc, item) => {
						...
						const timestamp = created_at instanceof Date ? created_at.getTime() : new Date(created_at).getTime();
