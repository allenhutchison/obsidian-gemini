...
- The management UI doesn't currently include a frontmatter-filter editor — set the `frontmatterFilter:` block by hand-editing the hook's markdown file. If a filter key contains a colon, an apostrophe, or a leading `#`/`[`/`{`, wrap it in single quotes (`'my: key': value`) so the frontmatter block stays parseable — an unparseable block makes the hook disappear from the manager rather than fail loudly. The plugin writes every string field quoted this way when it saves a hook, so re-saving from the UI normalizes the quoting for you.
