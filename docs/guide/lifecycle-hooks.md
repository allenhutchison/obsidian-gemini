...
- The management UI doesn't currently include a frontmatter-filter editor — set the `frontmatterFilter:` block by hand-editing the hook's markdown file. If a filter key contains a colon, an apostrophe, or a leading `#`/`[`/`{`, wrap it in single quotes so the frontmatter block stays parseable — an unparseable block makes the hook disappear from the manager rather than fail loudly. Inside single quotes, **an apostrophe is written twice**: a key of `Allen's: notes` is `'Allen''s: notes'`. If a value needs to contain a line break, use double quotes and `\n` (`"line one\nline two"`) — a line break inside single quotes is folded into a space. The plugin writes every free-text string field this way when it saves a hook, so re-saving from the UI normalizes the quoting for you. (`trigger` and `action` are fixed sets of known values, so they keep their own plain quoting.)
