...
each list Gemini, Ollama, and OpenAI; Web search, Deep research, Vault search index, and Image
...
- Choosing OpenAI for a feature means that feature's requests go to `api.openai.com` (or
  whatever base URL you've configured) instead.
...
- Each provider card only needs its own credential: the Gemini card's API key is required only
  ...
  the OpenAI card's API key field works the same way for OpenAI.
...
switching between them costs nothing extra.
