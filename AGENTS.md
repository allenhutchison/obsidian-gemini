...
npm run check:fields   # Advisory unread-field scan (per-field reachability over src/ interface and type-alias fields; #1469). Always exits 0 — the report lands in the Unread-field workflow artifact. Suppress a genuinely write-only field with 'wiring:keep' on its declaration line. Flip to blocking after the noise rate has been measured for a release.
...
The plugin uses a factory pattern (`ModelClientFactory` in `src/api/factory.ts`) to create model API clients, wrapped with a retry decorator (`RetryDecorator`) for resilience. Since the settings redesign, routing is a dense table keyed by feature: `settings.features: Record<FeatureId, { provider, model }>` (`FeatureId` = `chat`, `summary`, `completions`, `rewrite`, `webSearch`, `deepResearch`, `rag`, `imageGen`), with `settings.defaultProvider` never serving a request: it only **seeds** entries missing at settings load (`sanitizeFeatureRoutes` in `src/main.ts`), and otherwise feeds display/re-init helpers. The factory resolves each call's provider via `featureProvider(settings, featureId)` in the leaf module `src/api/feature-routing.ts`, which returns `null` when the feature is off (`provider === 'none'`) or unsupported/unconfigured — **there is no silent fallback**; an unservable feature stays off rather than being re-routed to another provider — then instantiates the `ModelApi` implementation for the resolved `ModelProvider` (one client class per provider id; the union lives in `src/api/providers/registry.ts`). The model comes from `resolveFeatureModel(settings, featureId)` in `src/models.ts`, which falls back to the provider's role default only when the stored value is `''`. All API implementations follow the `ModelApi` interface. Retry logic (exponential backoff) is a fixed policy in `RetryDecorator`, not settings-driven. Each provider lives in its own package under `src/api/providers/{gemini,ollama,openai,anthropic}/`; the capability matrix lives in `src/api/providers/registry.ts` and routing helpers live in `src/api/feature-routing.ts`. Decoding of incoming conversation history — the Gemini `Content` shape plus the legacy `{ role, text | message }` entry — is shared by the non-Gemini clients in the leaf module `src/api/providers/history-walk.ts`; `OllamaClient`/`OpenAIClient`/`AnthropicClient` call `walkHistoryEntry()` (Anthropic passing `{ acceptsPdf: true }`) and then only _emit_ their own message type, so the provider-agnostic half cannot drift between them (the Gemini client speaks `Content` natively and has no such step). Two shared cross-provider leaves deliberately live at the `src/api/providers/` package root, outside the per-provider packages: `history-walk.ts` (above) and `tool-id-ledger.ts` (called by `AnthropicClient` and `OpenAIClient`).

### Key Components

1. **API Layer** (`src/api/`): Factory pattern (`ModelClientFactory`) for creating provider-appropriate model API clients, decorator pattern (`RetryDecorator`) for resilience, and interface abstraction (`ModelApi`) for consistent API interactions. Provider-specific code is encapsulated under `src/api/providers/{gemini,ollama,openai,anthropic}/`.
2. **Feature Modules**: Separate modules for chat, completions (`completions.ts`), summary (`summary.ts`), and rewrite (`rewrite.ts`)
3. **Context System** (`src/files/index.ts`, `ScribeFile`): Builds context from the active file or explicit file list (no link traversal — the agent follows links dynamically via the `read_file` tool)
...
- Lint with ESLint (`npm run lint`, autofix with `npm run lint:fix`): the `eslint-plugin-obsidianmd` recommended preset enforces Obsidian-specific best practices (memory-leak prevention, cross-window safety, command-ID conventions, manifest validation, etc.). Pervasive rules that need broader refactoring are temporarily disabled in `eslint.config.mjs` — see the `PERVASIVE_OBSIDIANMD_RULES_TODO` block for follow-up items. One repo-specific `no-restricted-syntax` entry (`PATH_CONTAINMENT_RULE`) flags hand-rolled path containment through two selectors — the concatenation passed straight to `.startsWith()` (`p.startsWith(folder + '/')`, #1402) and the same concatenation hoisted into a local first (`const prefix = folder + '/'`, #1482), which the first selector cannot see — and points at `isPathInFolder()` in `src/utils/file-utils.ts`, which owns that predicate and is the rule's only exempt file; a deliberate strict-descendant site takes a line-scoped disable carrying the reason, and both selectors share the one rule so that per-rule disable still suppresses only its own site. A third entry (`PROVIDER_LITERAL_RULE`, #1317) fails any string literal `'gemini'` / `'ollama'` / `'openai'` / `'anthropic'` outside the modules that own provider identity (`src/api/providers/**`, the `src/api` routing/credential/status leaves, `factory.ts`, `models.ts`, `types/`) — non-provider modules must ask the registry (`getCapabilities`/`featureProvider`/`PROVIDER_IDS`) instead of branching on provider names; pre-existing sites carry line-scoped disables with reasons that come out as #1308/#703 clear them, and the exemption file list plus per-rule disable caveat live in the comment on the rule in `eslint.config.mjs`. A second repo-specific entry (`no-tags-as-reachability`, a local rule in the same file) fails any `@public`/`@beta`/`@alias` JSDoc tag under `src/` — the plugin has no public API and the tags would exempt exports from the knip gate (#1525).
