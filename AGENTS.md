...
The **auto-dev** skill (`.agents/skills/auto-dev/SKILL.md`) runs one tick of an unattended issue-to-PR pipeline: it triages open issues for readiness (asking clarifying questions where needed), posts implementation plans for maintainer approval, builds the oldest approved issue into a PR, and addresses CodeRabbit/human review feedback on the open automated PR. It **never merges** — merging is always the maintainer's act — and it keeps at most `MAX_PRS_IN_FLIGHT` automated PRs (default 6) in flight at once, all awaiting the maintainer's merge.
