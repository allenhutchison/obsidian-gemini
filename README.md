...
- **Scheduled Tasks:** Automate recurring AI prompts — daily summaries, weekly reports, periodic vault maintenance — without manual intervention. Each task is a markdown file with a frontmatter schedule (`daily`, `weekly`, `interval:Xm`, etc.) and a prompt body. Tasks run as headless agent sessions and write output to your vault. Supports per-task model and tool-category overrides, automatic pause after repeated failures, and a task monitor via the command palette.
